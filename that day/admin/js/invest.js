...
fetch(`./php/invest.php?i=${user}`, { method: "GET" })
  .then((res) => res.json())
  .then((data) => {
    console.log(data);
    if (data[0] == " " || data[0] == "" || data[0] == null) {
      document.getElementById("inid").innerHTML = "<h1 style='text-align:center'>No Investment</h1>";
    //   document.getElementById("hid").innerHTML = "";
      ...
      for (let i = 0; i < data.length; i++) {
        document.getElementById("inid").innerHTML += `
           ...
           <td data-label="Amount">N ${data[i].amount}.00</td>
           <td data-label="Date Started">${data[i].date}</td>
           <td data-label="Status">${data[i].confirm == 0 ? "Pending" : "Paid"}</td>
